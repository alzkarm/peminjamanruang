...
export class AuthService {
  ...
  async login(loginDto: LoginDto) {
    ...
    // Step 1: Verify credentials against LDAP pdc.yarsi.ac.id or local password fallback
    ...
    try {
      ...
      this.logger.warn(`LDAP authentication unavailable or failed for ${cleanUsername}: ${error.message}. Checking local database...`);
    }
    ...
    if (!isLdapSuccess) {
      if (user && user.passwordHash) {
        ...
        throw new UnauthorizedException('Gagal terhubung ke LDAP YARSI dan akun lokal tidak ditemukan.');
      }
    }

    if (!user && isLdapSuccess) {

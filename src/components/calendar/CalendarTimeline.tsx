                            ...
                            {[
                              ...
                              return (
                                <button key={`booking-${item.booking.id}`} type="button" onClick={() => openBooking(item.booking)} className={`flex min-h-14 w-full items-center gap-3 rounded-xl border px-3 py-2.5 text-left ${tone.surface}`}>
                                  ...
                                  <span className="min-w-0 flex-1"><span className="block text-sm font-bold">{item.booking.startTime}–{item.booking.endTime}</span><span className="block truncate text-xs opacity-75">{item.booking.title || tone.label}</span></span>
                      ...
                      ...roomBookings.map((booking) => {
                        ...
                        return (
                          <button key={`booking-${room.id}-${booking.id}`} type="button" onClick={() => openBooking(booking)} className={`z-10 m-1 overflow-hidden rounded-lg border p-2 text-left shadow-sm hover:shadow-md ${tone.surface}`} style={{ gridColumn: roomIndex + 2, gridRow: `${placement.row} / span ${placement.span}` }}>
                            <span className="flex items-center justify-between gap-2 text-[9px] font-extrabold uppercase tracking-wide"><span className="truncate">{booking.title || tone.label}</span><span className={`h-2 w-2 shrink-0 rounded-full ${tone.marker}`} aria-hidden="true" /></span>

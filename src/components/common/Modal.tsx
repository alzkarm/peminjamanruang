      ...
      >
        {title ? (
          ...
          <button
            ...
            onClick={onClose}
            aria-label="Tutup dialog"
            className="absolute right-3 top-3 z-20 flex min-h-11 min-w-11 items-center justify-center rounded-lg border border-white/15 bg-black/15 text-white backdrop-blur hover:bg-black/25"
          >
            <X className="h-5 w-5" />
          </button>
        )}

        <div className={`max-h-[min(84vh,780px)] overflow-y-auto overscroll-contain ${title ? 'p-4 sm:p-6' : 'p-0'}`}>{children}</div>
      </div>
    </div>
  );
}

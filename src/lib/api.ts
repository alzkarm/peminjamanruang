...
export function mapBackendRoomToFrontend(r: any): Room {
  ...
  const floorName = typeof r.floor?.name === 'string' ? r.floor.name : String(floorLevel);
  ...
  return {
    ...
    locationDetails: `${building}, Lantai ${floorName}`,
  };
}
...
export const roomsApi = {
  async getAll(query?: { isSpecialRoom?: boolean; floorId?: number; search?: string }): Promise<Room[]> {
    const params = new URLSearchParams();
    if (query?.isSpecialRoom !== undefined) {
      params.append('isSpecialRoom', String(query.isSpecialRoom));
    }
    if (query?.floorId !== undefined) {
      params.append('floorId', String(query.floorId));
    }
    if (query?.search) {
  ...
  async checkAvailability(roomId: string, startTime: string, endTime: string): Promise<RoomAvailabilityResponse> {
    return request<RoomAvailabilityResponse>(
